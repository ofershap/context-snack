...
**Curated tier (superhuman, rundown, tldr):** scrape already-edited newsletter archives (human-written headlines/summaries), not raw firehoses. `rundown.ts` intentionally leaves `summary` unset so the existing `enrichWithOg` og:description pipeline fills it from the linked article page (one URL = one story there, so og:image/description are accurate).

**Superhuman AI is different (v1.3.2):** each `/p/<slug>` URL is a multi-story daily digest, not a single-topic article — the archive headline and the page's own og:image only represent *one* of several stories on that page. Pulling og:description/og:image for a card built from a different headline caused real mismatches (a jet-speed-record title paired with an orca video thumbnail; a Kimi K3 story paired with a random stock photo). `superhuman.ts` instead parses the numbered `"N. Headline: two sentences"` items out of each issue's body directly — those are already self-contained BLUF stories — and produces one card per item, with `summary` set at scrape time. It's excluded from `enrichWithOg`'s image step entirely (`NO_IMAGE_ENRICHMENT_SOURCES` in `enrich.ts`) since a digest page has no single image that fairly represents every story pulled from it; these cards render text-only.
